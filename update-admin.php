...
if (isset($_POST['ubah_foto'])) {
    ...
    $fotobaru = $nama . "-" . $_FILES["foto"]["name"];
		...
		$data = mysqli_fetch_array(mysqli_query($connect, "SELECT * FROM perangkat_desa WHERE id_perangkat_desa = '$id_admin'"));
		...
		mysqli_query($connect, "UPDATE admin SET
			...
			nama = '$nama',
			username = '$username',
			foto = '$fotobaru' WHERE id_admin = '$id_admin'");
		echo '<script language="javascript">swal("Update berhasil!", "Data Admin terupdate", "success").then(() => { window.location="kelola-admin"; });</script>';
	...
	mysqli_query($connect, "UPDATE admin SET
		id_admin = '$id_admin',
		nama = '$nama',
		username = '$username' WHERE id_admin = '$id_admin'");
	echo '<script language="javascript">swal("Update berhasil!", "Data Admin terupdate", "success").then(() => { window.location="kelola-admin"; });</script>';
