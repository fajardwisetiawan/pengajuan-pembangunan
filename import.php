...
<?php
	...
	if($sheets_jenkeg=='' || $sheets_nrab=='' || $sheets_judkeg=='' || $sheets_deskeg=='') {
		echo '<script language="javascript">swal("Simpan gagal!", "Ada kolom yang kosong", "warning").then(() => { window.location="form-import"; });</script>';
	}else{
		...
		if ($i > 3) {
			if($sheets_jenkeg=='' || $sheets_nrab=='' || $sheets_judkeg=='' || $sheets_deskeg=='') {
				...
				$sql = 'INSERT INTO proposal_pengajuan SET';
				$sql .= ' jenis_kegiatan="'.$sheet['B'].'",';
				$sql .= ' nilai_rab="'.$sheet['C'].'",';
				$sql .= ' judul_kegiatan="'.$sheet['D'].'",';
				$sql .= ' deskripsi_kegiatan="'.$sheet['E'].'"';
