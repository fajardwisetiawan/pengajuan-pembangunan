...
$deskripsi_gambar = antiinjection($_POST['deskripsi_gambar']);
...
if($_FILES['foto']['name']!='')
	{
		...
		if($check !== false) {
			...
			echo '<script language="javascript">swal("Simpan gagal!", "File bukan gambar", "warning").then(() => { window.location="pembangunan-dalam-proses"; });</script>';
				$uploadOk = 0;
		}

		// Check file size
		if ($_FILES["foto"]["size"] > 2000000) {
			echo '<script language="javascript">swal("Simpan gagal!", "Gambar tidak boleh lebih dari 2MB", "warning").then(() => { window.location="pembangunan-dalam-proses"; });</script>';
				$uploadOk = 0;
		}
		// Allow certain file formats
		if($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg") {
			echo '<script language="javascript">swal("Simpan gagal!", "Gambar hanya boleh berupa JPG, PNG atau JPEG", "warning").then(() => { window.location="pembangunan-dalam-proses"; });</script>';
				$uploadOk = 0;
		}
		// Check if $uploadOk is set to 0 by an error
		if ($uploadOk == 0) {
			echo '<script language="javascript">swal("Simpan gagal!", "Foto gagal diupload!", "warning").then(() => { window.location="pembangunan-dalam-proses"; });</script>';
		// if everything is ok, try to upload file
				...
				if (move_uploaded_file($_FILES["foto"]["tmp_name"], $target_file)) {
					...
					gambar_kegiatan = '$fotobaru' WHERE id_proposal = '$id'");
					echo '<script language="javascript">swal("Simpan berhasil!", "Foto berhasil ditambahkan", "success").then(() => { window.location="pembangunan-dalam-proses"; });</script>';
